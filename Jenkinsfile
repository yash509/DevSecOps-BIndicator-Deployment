pipeline {
    ...
    environment {
        IMAGE_NAME = "yash5090/bindicator"
    ...
    stages {
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop bindicator | true"
                sh "docker rm bindicator | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name bindicator -p 5000:80 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service bindicator-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"bindicator\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
